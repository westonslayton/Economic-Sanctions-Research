...
counter <- 1
while (counter < 11) {
    df <- rbind(df, data.frame(get.Comtrade(r = "842", p = country_codes[[counter]], ps = "202001,202002,202003", freq = "M",rg = 2)))
    counter <- counter + 1
}
...
wbsearch("Total Export") |> view()
str(wb_cachelist, max.level = 1)
wb_indicators <- wb_cachelist$indicators |> view()
```

```{r}
#|: using-wbstats-pkg-to-get-GDPs
#in US dollars + years

gdp <- c("GDP" = "NY.GDP.MKTP.CD")
gdp_per_country <- wb_data(gdp, country = "all", mrv = 1) |> 
