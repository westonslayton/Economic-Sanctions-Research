...
To query export and import data from Comtrade, you can use the following methods:
  ...
  * ```get_cmd_imports```: writes csv file containing import data for a commodity for a specific reporter-partner pair
  *** If you want the total amount of a commodity imported by a reporter, make the partner ```0``` (the world). If you want the amount of a commodity imported by a reporter from each of its individual partners, make the partner argument ```None``` (all possible partners).
