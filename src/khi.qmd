...
# beta1 <- read_excel(path) #Import external trade and GDP data
# beta1$A = beta1$Tkj / beta1$Tj            #Create A (see p. 28 Bali&Rapelanoro (2021))
# beta1$B = beta1$Tj / beta1$Yj             #Create B (see p. 28 Bali&Rapelanoro (2021))
# beta1$beta = beta1$A * beta1$B            #Create Beta
# colnames(beta1) = c("Date", "Tkj", "Tj", "Yj", "A", "B", "beta")
# beta1$Date <- as.Date(beta1$Date) #Transform column "Date" in date type
# view(beta1)
...
dates <- as.Date(dates)
dates <- as.double(date)
sanction_dates <- as.Date(sanction.list$Date)
view(sanction_dates)
view(dates)
x <- intersect(sanction_dates, dates) 
view(as.Date(x))
view(dates)
view(sanction_dates)
check <- sanction.list |> filter(Date %in% dates)
sanction.list <- check |> 
  mutate(
    Date <- as.Date(Date)
  )
