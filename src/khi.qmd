...
ggplot(data, aes(x=as.Date(dt_res$Date), y=S_index)) + geom_line(color="steelblue") + ylab("Value of the Sanction Index") + xlab("Date") + scale_x_date(date_breaks = "2 month", date_labels = "%m-%Y") + theme_bw() + theme(axis.text.x=element_text(angle=60, hjust=1))
path <- file.path(dirname(getwd()), "data")
ggsave(path = path, filename = "new_plot.png")
file <- "new_example.xlsx"
path <- file.path(dirname(getwd()), "data", file)
write_xlsx(dt_res, path)
```

```{r}
# Morad: fixed errors, will work on function in coming days, pls check graphs attached as well as Excel file w/ percent diffs to see if betas are close enough, can't get data for 2022+ b/c of World Bank (can't work around--tried other sources)
