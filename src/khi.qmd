...
load_beta <- function(reporter, partner, provide_beta, betas) {
  if (provide_beta) {
    ...
    for (year in dates_only) {
      ...
      if (as.double(year) > as.double(curr_year) - 1) {
          ...
          df <- everything(year)
          df <- df |> beta_filter(df, reporter, partner, year, NULL)
...
betas <- "austria_combo.xlsx" # only needed if above is TRUE; provided file must be in data folder
