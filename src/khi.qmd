...
file <- "old_france_index.xlsx"
path <- file.path(dirname(getwd()), "data", file)
old_index <- read_excel(path) 
old_index <- old_index |> select(Date, Sindex)
old_index <- old_index |>
  mutate(
    growth_rate = (Sindex - lag(Sindex)) / lag(Sindex)
  )

file <- "new_france_index.xlsx"
path <- file.path(dirname(getwd()), "data", file)
new_index <- read_excel(path)
new_index <- new_index |> select(Date, Sindex)
new_index <- new_index |>
  mutate(
    growth_rate = (Sindex - lag(Sindex)) / lag(Sindex)
  )
...
first <- as.Date(min(new_index$Date))
last <- as.Date(max(new_index$Date))
dates <- c(seq(ymd(first), ymd(last), "days"))
dates <- as.Date(dates)
old_index <- old_index |> filter(Date %in% dates)


ggplot(old_index, aes(x=as.Date(old_index$Date), y=growth_rate)) + geom_line(aes(color="Old Index Growth Rate")) + geom_line(data=new_index, aes(color="New Index Growth Rate")) + ylab("Value of the Sanction Index Growth Rate") + xlab("Date") + scale_x_date(date_breaks = "2 month", date_labels = "%m-%Y", limits = as.Date(c("2014-01-01", "2019-01-01"))) + theme_bw() + theme(axis.text.x=element_text(angle=60, hjust=1))


sd(old_index$growth_rate, na.rm = TRUE)
sd(new_index$growth_rate, na.rm = TRUE)
...
```{r}
# beta check (to be removed later)

# file <- "beta_daily.xlsx"
# path <- file.path(dirname(getwd()), "data", file)
# old_beta <- read_excel(path)
# old_beta$A = old_beta$Tkj / old_beta$Tj           
# old_beta$B = old_beta$Tj / old_beta$Yj             
# old_beta$beta = old_beta$A * old_beta$B           
# colnames(old_beta) = c("Date", "Tkj", "Tj", "Yj", "A", "B", "beta")
# beta$Date <- as.Date(beta$Date)
# old_beta <- old_beta |> mutate(old_beta = beta) |> select(Date, old_beta)
# 
# file <- "combo.xlsx"
# path <- file.path(dirname(getwd()), "data", file)
# new_beta <- read_excel(path) #Import external trade and GDP data
# new_beta <- new_beta |> mutate(new_beta = beta) |> select(Date, new_beta)
# 
# beta <- inner_join(old_beta, new_beta, by = "Date")
# beta <- beta |> mutate(
#   percent_diff = abs(old_beta - new_beta) / old_beta * 100
# )
# beta |> summarize(mean_diff = mean(percent_diff)) # ~11%
# view(beta)
# 
# file <- "beta_check.xlsx"
# path <- file.path(dirname(getwd()), "data", file)
# write_xlsx(beta, path)
```
