...
sanction.list <- read_excel("sanction_list.xlsx") #Import the sanction list.
...
beta <- read_excel("beta_daily.xlsx") #Import external trade and GDP data
...
if (nrow(dt2)<nrow(khi)){     #Test to see if dt2_m has less rows than khi 
  nb_l <- nrow(dt2)
...
ggsave("my_plot_check.png")
write_xlsx(dt_res, "example.xlsx")
```
