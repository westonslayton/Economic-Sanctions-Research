...
beta_filter <- function(df, reporters, partners, keyword) {
  ...
  if (length(pull(df, date)) < 364) {
    print("Missing dates have been detected--the missing data will be linearly interpolated.")
    year <- 2017
    ...
    df <- df |> 
      mutate(beta = coalesce(beta, predict(lm(beta ~ date), across(date)))) |> 
...
year <- "2017" # year for which you want to gather data
...
first_day <- as.Date("2014-01-01")
last_day <- as.Date("2021-12-31")
Date <- c(seq(ymd(first_day), ymd(last_day), "days"))
Date <- as.data.frame(Date)

df <- left_join(Date, beta)
df <- df |> 
  mutate(beta = coalesce(beta, predict(lm(beta ~ Date), across(Date)))) |> 
  mutate_if(is.numeric, round, digits = 9)

path <- file.path(dirname(getwd()), "data", "combo.xlsx")
write_xlsx(df, path) 
```
