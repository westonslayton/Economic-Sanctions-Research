...
everything <- function(year) {
  ...
  wb_gdp <- get_wb(10, "gdp")
  wb_total_exports <- get_wb(10, "exports")
  final <- beta_calc(
...
filtered_final <- beta_filter(final, eu_reporters, partners, paste0("eu", year))
# view(filtered_final)
```

```{r message = FALSE, warning = FALSE}
#temp chunk to get all data for Austria sanction_list check (delete later)
...
) |> filter(reporter == "Austria" & partner == "Russia") |> view()
...
combo <- rbind(year2014, year2015, year2016, year2017, year2018, year2019, year2020, year2021)
combo$Date <- format(as.Date(combo$date, format = "%Y-%m-%d"), "%d/%m/%Y")
view(combo)
write_xlsx(
  combo,
  file.path(
