...
add_countries <- function(clean) {
  ...
  combo$date <- as.Date(combo$date, origin = "1970-01-01")
  return(combo)
}
```
...
clean_imf <- function(file) {
  ...
  if (str_sub(file, -1) == "M") {
    ...
    clean$date <- as.Date(clean$date, origin = "1970-01-01")
  } else {
...
get_wb <- function(years, df_to_return) { 
  ...
  both$date <- as.Date(both$date, origin = "1970-01-01")
  ...
  if (df_to_return == "gdp") return(gdp)
...
# pairs_monthly <- comtrade_pairs_monthly
# pairs_annual <- comtrade_pairs_annual
# total_monthly <- imf_total_monthly
# gdp <- wb_gdp
# view(wb_gdp)
...
beta_calc <- function(pairs_monthly, pairs_annual, total_monthly, gdp) {
  ...
  total <- data.frame(
    date = as.Date(character(), format = "%Y-%m-%d", origin = "1970-01-01"),
    reporter = character(),
...
filtered_final <- beta_filter(final, reporters, partners, paste0("eu", year))
# view(filtered_final)
```
