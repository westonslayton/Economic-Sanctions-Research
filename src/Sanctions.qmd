...
final <- merge(comtrade_pairs_monthly, comtrade_pairs_annual, imf_total_monthly, wb_gdp)
# view(final)
```

```{r}
#|: freq-conv-and-beta-calc
# make into a function 
# combine this function w/ merge function to make a SUPER function that takes in the dfs and magically spits out merged, freq-converted, beta-calculated SUPER df
# need to filter resulting df b/c has too many rows to view entire thing @ once
...
total |> filter(reporter == "Zimbabwe") |> view()
```
