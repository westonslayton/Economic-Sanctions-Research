...
# why isn't france in pairs? need to add countries missing from comtrade_annual to comtrade_monthly, intersect w/ imf countries to then get all countries that can be added, then loop over this vector with imf function -> change the functionality of functions in package
comtrade_pairs_annual |> distinct(reporter) |> view()
comtrade_pairs_monthly |> distinct(reporter) |> view()

# need these changes to df colnames before merge
pairs <- comtrade_pairs_monthly |> mutate(partner_exports = value)
total <- imf_total_monthly |>
  mutate(total_exports = value) |>
  select(date, total_exports, reporter)
gdp <- wb_gdp

# getting all common dates and filtering dfs for these dates only
p_dates <- pairs |> distinct(date)
t_dates <- total |> distinct(date)
g_dates <- gdp |> distinct(date)
exp_only_dates <- intersect(p_dates, t_dates)
gdp_only_dates <- intersect(exp_only_dates, g_dates)
view(gdp)
view(pairs)
view(total)
...
check <- pairs[pairs$date %in% exp_only_dates]
view(pairs)
view(total)
view(gdp)

# adjusting countries w/ diff names to be same across all dfs
# be sure to filter NAs out of merged df
pairs <- pairs |> 
  mutate(
    reporter = case_when(
