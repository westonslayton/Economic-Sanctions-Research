...
```{r}
#|: function-to-add-countries-in-comtrade-annual-df-to-comtrade-monthly-df
...
# add_annual <- function(comtrade_pairs_annual, comtrade_pairs_monthly) {
  annual <- comtrade_pairs_annual |> distinct(reporter)
  monthly <- comtrade_pairs_monthly |> distinct(reporter)
  diff <- setdiff(annual$reporter, monthly$reporter)
  diff_df <- comtrade_pairs_annual |> filter(reporter %in% diff)
  diff_df <- diff_df |> mutate(value = value / 12)
  diff_dupe <- diff_df[rep(1:nrow(diff_df), each = 12), ]
  year <- year(as.Date(comtrade_pairs_annual$date[1]))
  date <- c(
  ...
  date <- as.Date(date)
  num_countries <- length(diff_df$reporter)
  date <- rep(date, len = length(diff_dupe$date))
  view(date)
  diff_dupe$date <- date
# done w/ fixing annual df, just need to add diff_dupe to monthly df and then return!
# }
```
...
# process of data querying, cleaning + merging will need to be repeated for another year
...
pairs_annual <- comtrade_pairs_annual |>
  mutate(partner_exports = value) |>
  select(date, partner_exports, reporter, partner)
...
pairs_annual <- adjust_names(pairs_annual)
total <- adjust_names(total)
gdp <- adjust_names(gdp)

# adding (and converting) data from annual exp-pair df to monthly exp-pair df
...
# 1) filter for countries shared by all dfs
# 2) left_join total-exports df to per country-exp df by reporter + date
# 3) adjust gdp df to monthly (divide by 12 only--don't adjust dates)
# 4) left_join gdp df to export df (from prev merge) by reporter only
# 5) filter NAs out of merged df (ie, countries that aren't in all dfs)
### the resulting df should be ready for freq conv (steps below) ###
```
...
```{r}
# OLD check of above function -> scrap later
