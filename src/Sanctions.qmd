...
RETICULATE_PYTHON = virtualenv_python("my_environment")
use_virtualenv("my_environment")
virtualenv_install(
  "my_environment", 
  c("export_ease", "comtradeapicall", "pandas", "requests", "flatten_json")
)
```
...
Comtrade.get_all_exports("B", 2021) # writes csv file containing all country-pair export data
```
...
Comtrade.get_total_exports("B", 2021) # writes csv file containing total-export values for each country
```
...
year <- "2021" # year of data you're querying
...
file_name <- "comtrade_total_exports_"

comtrade_total_annual <- read_csv(
  file.path(
    dirname(getwd()),
    "data",
    paste0(file_name, year, "A", ".csv")
...
comtrade_total_monthly <- read_csv(
  file.path(
    dirname(getwd()),
    "data",
    paste0(file_name, year, "M", ".csv")
...
IMF.get_total_exports("B", 2021) # writes csv file containing total-export values for each country
```
...
IMF.get_reporter_exports("France", "B", 2021) # writes csv file with export data for reporter to all partners
```
...
# file names must be changed depending on queries (year, freq)
# in USD + months/years (depends on query)
# only run when you're loading queried data for the first time (and need to clean it)
...
country <- "FR" # country of data you're querying\
file_name <- "all_exports_"

imf_fr_all_annual <- read_csv(
  file.path(
    dirname(getwd()),
    "data",
    paste0("imf_", country, "_", file_name, year, "A", ".csv")
  )
)
imf_fr_all_monthly <- read_csv(
  file.path(
    dirname(getwd()),
    "data",
    paste0("imf_", country, "_", file_name, year, "M", ".csv")
  )
)
```
...
# first argument specifies how many years you want data for, starting at the most recent year that has complete data (e.g., get_wb(3, ...) will get data for 2019, 2020, and 2021)
# second arguments specifies data frame to return; valid args = "gdp" or "exports"
...
num_years <- "3" # of years of data you queried
file_name <- "wb_gdp"
...
file_name <- "wb_total_exports"

wb_total_exports <- read_csv(
  file.path(
    dirname(getwd()),
    "data",
    paste0(file_name, num_years, ".csv")
...
# when adding annual data (reporters + date), divide by 12, then extend seq of first days of all months, then replace old date col w/ new sequence
...
```{r}
#|: merge-convert-and-calc-beta

final <- beta_calc(comtrade_pairs_monthly, comtrade_pairs_annual, imf_total_monthly, wb_gdp)
```

```{r}
#|: function-to-filter-df-based-on-criteria
# can't view entire resulting df @ once or write Excel file b/c it's too large,
# so must filter the df first before viewing/writing a file
...
# can leave either reporters OR partners parameters NULL in order to get all reporters/partners

beta_filter <- function(df, reporters, partners, keyword) {
...
# note that beta_filter() can't write Excel files for dfs w/ > 1,000,000 rows, 
# though the function will continue to work otherwise even if this is the case

partners <- c("Russia")
eu_reporters <- c("Austria", "Belgium", "Bulgaria", "Croatia", "Republic of Cyprus", "Czech Republic", "Denmark", "Estonia", "Finland", "France", "Germany", "Greece", "Hungary", "Ireland", "Italy", "Latvia", "Lithuania", "Luxembourg", "Malta", "Netherlands", "Poland", "Portugal", "Romania", "Slovakia", "Slovenia", "Spain", "Sweden")

filtered_final <- beta_filter(final, eu_reporters, partners, "eu")
# view(filtered_final)
```
